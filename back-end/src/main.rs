...
async fn upload_chunk(mut payload: Multipart, file_path: String) -> Result<Option<bool>, io::Error> {
    let mut chunk_count = 0;
    while let Ok(Some(mut field)) = payload.try_next().await {
        let filepath = format!("{}/{}.part", file_path, chunk_count);
...
async fn finalize_upload(filename: &str, upload_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chunk_num = 0;
    while std::path::Path::new(&format!("{}/{}.part", upload_dir, chunk_num)).exists() {
        let chunk_path = format!("{}/{}.part", upload_dir, chunk_num);
...
async fn upload(req: HttpRequest, payload: Payload) -> HttpResponse {
    ...
    if let Ok(Some(upload_result)) = upload_chunk(multipart, upload_dir.to_string()).await {
        if upload_result {  
            if let Err(err) = finalize_upload(&info.filename, upload_dir).await {
                return HttpResponse::InternalServerError().body(err.to_string());
            }
            if info.chunk_index == info.total_chunk {
                match std::fs::remove_file(format!("{}/{}.part", upload_dir, 0)) {
